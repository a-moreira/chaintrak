...
use tokio_stream::{StreamExt, Stream, StreamMap};
...
async fn get_stream() -> anyhow::Result<impl Stream<Item = (&'static str, Event)>> {
    ...
    let mut stream = StreamMap::<&'static str, Pin<Box<dyn Stream<Item = Event>>>>::new();
...
where
    S: Stream<Item = (&'static str, Event)> + Unpin
{
    ...
    while let Some((_source, event)) = stream.next().await {
        match event {
            Event::Kick => {
